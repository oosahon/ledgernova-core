...
import shortTermLoanLiabilityLedgerEntity from '../00-short-term-loan.entity';
import {
...
describe('Short Term Loan Liability Entity', () => {
  ...
  describe('getCode', () => {
    it('should generate the next sub-ledger code for short term debt accounts', () => {
      expect(shortTermLoanLiabilityLedgerEntity.getCode('200000')).toBe(
        '200001'
      );
      expect(shortTermLoanLiabilityLedgerEntity.getCode('200099')).toBe(
        '200100'
      );
    });
  });
  ...
  describe('makeCreditCardAccountMeta', () => {
    ...
    it('should successfully create credit card account meta with all valid fields', () => {
      const meta =
        shortTermLoanLiabilityLedgerEntity.makeCreditCardAccountMeta(validMeta);
    ...
    it('should throw AppError if cardIssuer is too short', () => {
      expect(() =>
        shortTermLoanLiabilityLedgerEntity.makeCreditCardAccountMeta({
          ...validMeta,
          cardIssuer: 'A',
        })
      ).toThrow(AppError);
  ...
  describe('makeCreditCardAccount', () => {
    ...
    it('should successfully create a credit card account', () => {
      const [account, events] =
        shortTermLoanLiabilityLedgerEntity.makeCreditCardAccount(
          validPayload,
          '200000'
        );
  ...
  describe('makeOverdraftAccountMeta', () => {
    ...
    it('should successfully create overdraft account meta', () => {
      const meta =
        shortTermLoanLiabilityLedgerEntity.makeOverdraftAccountMeta(validMeta);
      expect(meta).toEqual(validMeta);
      expect(Object.isFrozen(meta)).toBe(true);
    });
    ...
    it('should throw AppError if linkedBankAccountId is invalid', () => {
      expect(() =>
        shortTermLoanLiabilityLedgerEntity.makeOverdraftAccountMeta({
          ...validMeta,
  ...
  describe('makeOverdraftAccount', () => {
    ...
    it('should successfully create an overdraft account', () => {
      const [account, events] =
        shortTermLoanLiabilityLedgerEntity.makeOverdraftAccount(
          validPayload,
          '200000'
        );
  ...
  describe('makeShortTermLoanAccountMeta', () => {
    ...
    it('should successfully create short term loan meta', () => {
      const meta =
        shortTermLoanLiabilityLedgerEntity.makeShortTermLoanAccountMeta(
          validMeta
        );
      expect(meta).toEqual(validMeta);
      expect(Object.isFrozen(meta)).toBe(true);
  ...
  describe('makeShortTermLoanAccount', () => {
    ...
    it('should successfully create a short term loan account', () => {
      const [account, events] =
        shortTermLoanLiabilityLedgerEntity.makeShortTermLoanAccount(
          validPayload,
          '200000'
        );
